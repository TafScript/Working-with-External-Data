...
breedSelect.addEventListener("change", async (event) => {
  ...
  try {
    const res = await axios.get(`/images/search`, {
      params: { breed_ids: selectedBreedId, limit: 10 }
    });
    ...
    for (let i = 0; i < images.length; i++) {
      const breedInfo = images[i].breeds[0]; // first breed object
      const item = Carousel.createCarouselItem(images[i].url, breedInfo.name, images[i].id, favourite);
    ...
    console.log("Problem receiving images: ", error);
  }
});
